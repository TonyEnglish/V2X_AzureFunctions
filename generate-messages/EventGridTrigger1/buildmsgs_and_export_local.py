...
def segmentToContainers(wzMapPt, numLanes, speedLimits):
    ...
    for index, node in enumerate(wzMapPt):
        ...
        logMsg('index: ' + str(index) + ', node: ' + str(node))
...
###
        commonContainer = wz_xml_builder_v22.buildCommonContainer(idList[1], wzStart, wzEnd, timeOffset, wzDaysOfWeek, c_sc_codes, newRefPt, 
...
###
        ...
        for rsz in reducedSpeedZones:
            ...
            rszContainer = wz_xml_builder_v22.buildRszContainer(speedLimit, rsz['geometry'], laneWidth)
            rszContainers.append(rszContainer)

        laneClosureContainers = []
        for laneClosure in laneClosureZones:
            laneClosureContainer = wz_xml_builder_v22.buildLaneClosureContainer(laneClosure['laneStat'], None, laneClosure['geometry'], laneWidth)
            laneClosureContainers.append(laneClosureContainer)

        situationalContainer = {}
        if workersPresentZones:
            situationalContainer = wz_xml_builder_v22.buildSituationalContainer(None, None, True, None)
...
###
    ...
    if maxHDOP > maxAllowableHDOP:
        logMsg('GPS Accuracy too low, max value of HDOP: ' + str(maxHDOP) +
...
###
    ...
    logMsg(str(laneType) + ', ' + str(len(pathPt)) + ', ' + str(len(appMapPt)) + ', ' +
           str(laneWidth) + ', ' + str(lanePadApp) + ', ' + str(refPtIdx) + ', ' + str(appMapPtDist))
    logMsg(str(laneStat) + ',' + str(wpStat) + ', ' + str(dataLane) +
...
def uploadLogFile():
    logFile.close()
    blob_client_log = blob_service_client.get_blob_client(
        container="logs", blob=logFileName.replace(".txt", ctrDT+".txt"))
